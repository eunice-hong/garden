...
- Jetpack Compose는 이 문제를 해결하기 위해 snapshot system을 사용하고 있으며,
- 이는 **Actor 모델**에 가깝고, 각각의 스레드가 상태의 복사본을 갖는 방식을 취함.
...
Jetpack Compose는 상태 스냅샷 시스템을 구현하기 위해 **MVCC (Multiversion Concurrency Control)**를 사용합니다.
...
- `Snapshot.takeSnapshot()`으로 생성된 Snapshot은 `ReadonlySnapshot`.
...
- Recomposer는 읽기/쓰기 추적을 통해 컴포지션의 변경 여부를 판단하고 **자동으로 recomposition** 트리거함.
...
- 각 subcomposition은 **자신만의 nested snapshot**을 생성함.
